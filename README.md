...
現時点のシステム名は **Deneuve 2** です。使用ボードは **Freenove ESP32-S3**です。Arduino IDE の設定は **Flash Size: 16MB / PSRAM: OPI PSRAM（8MB）** です。詳細品番・USB 設定は実物を確認して確定します。
...
詳しくは [機構仕様](docs/mechanism.md) を参照してください。シリアル確認用の初期スケッチと個別操作用のテスト版があり、自動捕獲制御は未実装、実機の誘引・検出・捕獲性能は未検証です。

## ハードウェアテスト版

提供されたプログラムを `firmware/hardware_test/hardware_test.ino` に保存しています。ESP32 コア3.3.11・ESP32Servo 3.2.1を使用します。[操作方法と確認点](docs/hardware-test.md)を参照してください。検出から吸引への自動連動は未実装です。
