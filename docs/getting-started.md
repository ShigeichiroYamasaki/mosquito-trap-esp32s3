...
使用ボードは **Freenove ESP32-S3**です。製品の公式説明に従ってボードを選択します。使用者指定の設定は **Flash Size: 4MB / PSRAM: OPI PSRAM** です。詳細な製品品番・搭載メモリの実容量・USB 設定は確認予定です。CI は引き続き `ESP32S3 Dev Module` を汎用のコンパイル対象とし、実機設定の検証とは区別します。
...
Arduino IDE の **Tools → Flash Size → 4MB** と **Tools → PSRAM → OPI PSRAM** を選択してください。PSRAM 容量は別途確認する項目です。
