...
初期スケッチは毎秒のシリアルログのみを出力します。誘引・検出・吸引の機構は設計方針を記録した段階で、受光配列・LED・サーボの個別操作は[ハードウェアテスト版](./hardware-test.md)に収録しました。ヒーター制御と検出から吸引への自動連動は未実装です。Arduino IDE の設定は Flash Size: 16MB / PSRAM: OPI PSRAM（8MB） です。ボードの詳細品番・USB 設定は確認予定で、電源と周辺部品は未確定です。誘引効果・検出性能・捕獲性能はまだ評価していません。
