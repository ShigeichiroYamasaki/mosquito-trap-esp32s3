...
重曹とクエン酸で作るCO₂と、半日以上履いた靴下を36°Cのヒーターで加温する「人工皮膚」で蚊を誘引します。人工皮膚は10cm角のケースに収め、匂いは自然拡散させます。CO₂は別流路の5Vファンを555タイマーで4〜6秒間隔に動かして拡散します。噴出口はケースの近くに置き、靴下へ直接風を当てません。
...
![誘引に使用する靴下の着用写真](/photos/attractant-sock.png)

*誘引に使用する靴下（着用時）。使用後にヒーターで加温して人工皮膚として用います。*
...
Geierらの実験では皮膚臭は均一な流れが有効でした。この知見を踏まえ、人工皮膚とCO₂の流路を分離します。人工皮膚は自然拡散を基本とし、必要なら独立した弱い連続送風を検討します。自然拡散で均一な濃度が得られるとは限らず、効果は実測して評価します。

同じCO₂総供給量で連続・間欠拡散を比較し、周期・ON時間・風量・背景濃度を記録します。**蚊が接近する位置の濃度波形**を十分な応答速度の測定器で確認し、接近数・候補数・捕獲数を分けて評価します。

### 点滴バッグからのCO₂供給

クエン酸水溶液は点滴バッグから供給します。シリコンチューブを通して、重曹粉末を入れたペットボトル内へ滴下します。生成したCO₂は液面より上から別のシリコンチューブで取り出し、専用ファン側の開放混合部へ送ります。人工皮膚ケースはこの流路から分離します。

![点滴バッグから重曹入りペットボトルへ滴下し、CO₂をシリコンチューブで専用ファンへ送る構成](/detection/co2-drip-feed.svg)

[CO₂供給図を拡大表示](/detection/co2-drip-feed.svg)
...
3.3V系と5V系は共通GNDを使用します。ヒーター電源は5Vです。電源容量とヒーター温度制御は検討中です。
...
ここは機能構成図です。[NE555・IRLZ44Nによる4秒タイマーの部品値・配線](./hardware.md#fan-timer)を掲載しています。ヒーター電源は5Vで、温度制御回路は検討中です。
