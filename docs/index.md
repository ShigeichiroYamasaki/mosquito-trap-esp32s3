...
「蚊の形」ではなく、小さく局所的な影が連続して移動し、短時間で消える振る舞いを捉えます。左右のLEDを独立に追跡し、起動時の校正で受光点ごとの光量差を補正します。

![18点の配列で影の出現・移動・消失を追跡する](/detection/tracking.svg)

[3相フレーム・初期校正・時間追跡を図で読む](./detection-design.md)

現在は3相取得・校正・時間追跡を実装しています。左右照明の視差による奥行き判定は次期計画です。検出・捕獲性能は実機で未評価です。
