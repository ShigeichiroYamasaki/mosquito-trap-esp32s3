...
初期スケッチは毎秒のシリアルログのみを出力します。誘引・検出・吸引の機構は設計方針を記録した段階で、ヒーター・受光配列・サーボの制御は未実装です。Arduino IDE の設定は Flash Size: 未確定（16MBの可能性あり） / PSRAM: OPI PSRAM です。ボードの詳細品番・メモリ容量・USB 設定は確認予定で、電源と周辺部品は未確定です。誘引効果・検出性能・捕獲性能はまだ評価していません。
